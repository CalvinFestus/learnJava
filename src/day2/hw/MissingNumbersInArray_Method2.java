...
public class MissingNumbersInArray_Method2 {
	...
	private void getMissingNumbers (int[] numbers) {
		...
		size_of_new_array = maximum + 1;
		
		int[] result = new int[size_of_new_array];
		
		for (int i : numbers) {
			result[i] = 1;
		}
		
		for (int i = mininum; i < result.length; i++) {
			if(result[i]==0) {
